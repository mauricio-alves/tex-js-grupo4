...
<script>
import dbServices from '@/store/db/dbServices'

  
  export default {
    // checkServices: document.querySelectorAll('input[type="checkbox"]'),
        ...
        this.checkServices.forEach(item => {
          if (item.checked) {
            this.reservation.services.push({
              id: item.id,
              service: dbServices[item.id].service,
              price: dbServices[item.id].price })
          };
        });
